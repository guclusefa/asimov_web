...
module.exports = {
    afficher_fiche_eleve: function (req, res) {
        ...
        if (req.session.user_info !== undefined) {
            ...
            // remplir eleve
            if (id == 0) {
                // si admin afficher tout les eleves
                if (req.session.user_info.user_isAdministration == 1) {
                    model_eleves.lister(function (lesEleves) {
                        res.json({ titre, lesEleves })
                    })
                    // sinon afficher que moi
                } else {
                    model_eleves.ficher(req.session.user_info.user_id, function (lesEleves) {
                        res.json({ titre, lesEleves })
                    })
                }
                // step 2
                ...
                if (req.session.user_info.user_isAdministration == 1 || (id == req.session.user_info.user_id)) {
                            ...
                            if (req.session.user_info.user_isAdministration == 1) {
                                model_eleves.lister(function (lesEleves) {
                                    model_notes.lesClassesEleves(unEleve.user_id, function (lesCursus) {
                                        res.json({ titre, unEleve, lesEleves, lesCursus })
                                    })
                                })
                                // sinon afficher que moi
                            } else {
                                                                        ...
                                                                        model_notes.getMinEval(function (lesMinEval) {
                                                                            model_notes.getMaxEval(function (lesMaxEval) {
                                                                                model_notes.getAvgEval(function (lesAvgEval) {
                                                                                    ...
                                                                                    // si admin
                                                                                    if (req.session.user_info.user_isAdministration == 1) {
                                                                                        res.json({ titre, unEleve, unCursus, lesEleves, lesCursus, lesMatieres, bilanClasseT1, bilanClasseT2, bilanClasseT3, bilanClasseT4 })
                                                                                        // sinon afficher que moi
                                                                                    } else {
                                                                                        model_eleves.ficher(id, function (lesEleves) {
                                                                                            res.json({ titre, unEleve, unCursus, lesEleves, lesCursus, lesMatieres, bilanClasseT1, bilanClasseT2, bilanClasseT3, bilanClasseT4 })
